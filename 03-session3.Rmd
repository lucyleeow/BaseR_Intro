...
samps <- substring(files, 12, nchar(files))
...
1. Subset the first column of the `count` matrix from `x` (our `DGEList` object) and 'save' (assign) it to a variable called `sample1`.
  ...
  + Find the total sum of all counts for that sample. Compare this number with the corresponding (first) number in the `lib.size` column from the `samples`. Is it the same?
...
*Hint*: you will need to remove columns where the `TXCHROM` column is `NA`. Revisit section \@ref(miss) to review missing values.
