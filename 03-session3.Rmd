...
Each data file details the number of reads mapped to a gene for 1 sample. There is 1 data file for each sample. Within each data file, there are 3 columns - 'EntrezID', 'GeneLength' and 'Count.' The first few lines of one file is shown below:
...
Next, we will create our `DGEList` object. I have put my data files in a folder called "data" (within my working directory). Thus, I must specify `path = "data"`. Depending on where you have put your data files, you may need a different input to `path` or not have to include the `path` argument.

```{r}
x <- readDGE(files, path = "data", columns = c(1,3))
```

The object we created has 2 elements, one named `samples` and one named `counts`. You can take a look at each using `View()`:
...
Notice that you can use the vertical and horizontal scroll bars to view all of the data. Each row is a gene, denoted by it's EntrezID and each column is a sample. Each value in this matrix gives the counts for one gene and one sample. Recall also that `DGEList` specifies that the `counts` element of the list must be a numeric matrix. 

Notice that the column names are the file names. The `DGEList` object was created using the information in the "txt" files. The function `readDGE()` has used the file name as the name of each sample because we have not told `readDGE()` what the sample names are. It has therefore simply used the filenames. This makes logical sense as each file contains the count data for one sample.
...
Let's change them to be the sample names (see Table \@ref(tab:samps)).
