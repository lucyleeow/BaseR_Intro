...
```{r help, fig.cap="Accessing help files via 'help' tab.", out.width="500px", echo=FALSE}
...
Help files are very useful but can be difficult to interpret at first due to the technical language used. We won't get too much practice reading help files during this course but I would encourge you to try to use them when figuring out how to use a new function.
...
If we try to create a variable that starts with a number, R will return us an error:
...
A data structure can be thought of as a 'container' for data. There are a number of different data structures in R and each have differents specifications about how data is 'contained'. 
...
Note how the values in each file are separated by a different 'character' in the two file formats.

You can specify what using the `sep` input in `read.delim()`. E.g. if your file was a csv, you can read it in using:

```{r, eval=FALSE}
read.delim("file.csv", sep = ",")
```

We don't need to specify the `sep` for our file, which is a tsv, as the default separater `read.delim()` assumes is tab. Thus we only need to specify when we are reading in a file NOT separated by tabs.
...
We can further explore our dataframe with the function `str()` (structure):

```{r}
str(genes)
```

This tells us that `genes` is a dataframe. It also tells us what *data type* each column is.
...
For example, it makes sense to perform mathematical functions on numbers but not on words. It makes sense to look for a certain term, like a gene name, in a word but not in a number.

There are five basic types of data in R:

* `logical` - either TRUE or FALSE. This is useful for binary data like if patient has a disease or not.
...
Each number corresponds to a category. This information is also stored. R calls the categorical value each number corresponds 'levels'.
...
Factors can be difficult to work with, so we don't want our gene symbols to be 'labelled' as factors. We can do this by specifiying this when reading our data in:
...
Note that now the column `SYMBOL` is now a character ('chr').
...
There are a few things happening in the command above:

1. The code on the right side of `<-` is evaluated first. It returns a vector as we saw above.
2. This vector is assigned to a column in `genes` called `Count_2`. Since this column does not yet exist in the `genes` dataframe, a new column called `Count_2` is created first.
...
The last thing we will do this session is to write out our new dataframe using the function `write.table()`.
...
Now write out this new dataframe as a file called "Ses1_homework_output.tsv", as a tsv (tab separated values) file.
