...
The above code evaluates `3 * 4` and assigns the output to the variable `my_num` - effectivey 'overwritting' the previous value assigned to it.
...
A data structure can be thought of as a 'container' for data. There are a number of different data structures in R and each have different specifications about how data is 'contained'. 
...
For example, you can't add 'characters'. Thus this doesn't work because `"1"` and `"2"` are labelled as 'characters':
...
Note the quotes, which always signify the character type.
...
There is an order to which data label R will label a vector as. Of all the data types that exist in the vector, the data type of the element that is furtherest right, will be the "final" data type label for the vector.
...
Note that you could also REPLACE a column with the same notation. For example, if in the command above, the left side was `genes$Count`, the old column called `Count` would be REPLACED with the new vector of numbers created on the right side.
