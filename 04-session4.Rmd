...
The `~` at the start tells R that we want to model the samples using the 'expression' to the right of the `~`. The `0` at the start tells R not to include an intercept. We have choosen not to include an intercept because it is easier to interpret but including the intercept is just as valid. 
...
Contrasts let us ask specific questions, such as what is the difference between two specific groups. In our experiment, we are interested in the difference between each pairwise group. A contrast matrix can be made using the `makeContrasts()` function. Within this function we specify the name of specific contrasts and the formula for that contrast. For example, the `BasalvsLP` contrasts asks about the difference between the `Basal` and `LP` groups. Note that the name of the groups must be written exactly as they are in the column names of our design matrix above. The final input to the `makeContrasts()` function simply gives the names of each cell type group (recall from above that `colnames(design)` gives the names of each cell type group).

The output of our contrast matrix is shown below:
